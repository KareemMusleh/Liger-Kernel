...
class LigerNativeSparseAttentionFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, attn_mask=None, stride=1, padding=0, dilation=1, groups=1, sparse=False):
        ...
        if sparse:
            if scores_inf.dtype != torch.float32:
                raise RuntimeError("Liger sparse native sparse attention currently only supports fp32 input scores")
