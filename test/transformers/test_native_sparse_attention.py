...
from liger_kernel.transformers.functional import liger_native_sparse_attention
...
class TorchNativeSparseAttention(torch.nn.Module):
...
def test_native_sparse_attention_correctness(B, C_in, C_out, L, K, groups, bias, dtype, atol, rtol):
    set_seed(42)
    scores = torch.randn(B, C_in, L, L, device=device, dtype=dtype)  # input

    ref_attn = TorchNativeSparseAttention(
...
def test_native_sparse_attention_functional(B, C_in, C_out, L, K, groups, bias, dtype, atol, rtol):
    ...
    out1 = liger_native_sparse_attention(scores1, weight1, bias1, stride=1, padding=K // 2, groups=groups)
    out2 = ref_attn(scores2)
...
class TorchSparseNativeSparseAttention(TorchNativeSparseAttention):
    def forward(self, scores):
        B, C_in, L, _ = scores.shape
        mask = _make_mask(L, scores.device)
        inf = torch.tensor(-1e9, device=scores.device, dtype=scores.dtype)
...
def test_sparse_native_sparse_attention_correctness(B, C_in, C_out, L, K, groups, bias, dtype, atol, rtol):
    set_seed()
    scores = torch.randn(B, C_in, L, L, device=device, dtype=dtype)

    ref_attn = TorchSparseNativeSparseAttention(
        C_in=C_in, C_out=C_out, K=K, groups=groups, bias=bias, dtype=dtype, device=device
    )
...
def test_sparse_native_sparse_attention_functional(B, C_in, C_out, L, K, groups, bias, dtype, atol, rtol):
    set_seed()
    scores = torch.randn(B, C_in, L, L, device=device, dtype=dtype)
    ...
    out1 = liger_native_sparse_attention(scores1, weight1, bias1, stride=1, padding=K // 2, groups=groups, sparse=True)
    out2 = ref_attn(scores2)
